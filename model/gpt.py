...
class GPT(nn.Module):
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        # Token embeddings:
        # (batch_size, context_length, model_dim)
        token_embedded = self.word_embeddings(context)
        ...
        # Broadcasting adds positional embeddings to every batch item
        embedded = token_embedded + position_embedded

        # Apply all TransformerBlocks
        hidden = self.transformer_blocks(embedded)
        ...
        return torch.round(logits * 10_000) / 10_000
